...
class Entrega {
    ...
    static class Tema1 {
        ...
        static boolean exercici1(
            ...
            for (int i = 0; i < universe.length; i++) {
                for (int j = 0; j < universe.length; j++) {
                    p1 = (p.test(universe[i], universe[j]));
                    test = !p1 || q.test(universe[i]) && r.test(universe[i]);
                    if(!test){break;}
                }
                if(!test){break;}
            }
      ...
      /*  static boolean exercici1() {
            ...
            int[] universe = {2, 3, 5, 6};
            BiPredicate<Integer, Integer> p = (x, y) -> x * y <= 4;
            Predicate<Integer> q = x -> x <= 3;
            Predicate<Integer> r = x -> x <= 3;
            boolean test;
            boolean p1;
